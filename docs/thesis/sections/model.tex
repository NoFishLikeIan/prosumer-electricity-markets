...
\begin{equation}
    c(s, r) = \log \left( 1 + \exp(c_1 \cdot s \cdot r - c_0) \right)
\end{equation}

with derivative,

\begin{equation}
    \frac{\partial c}{\partial s} = \overbrace{\frac{c_1}{1 + \exp(c_0 - s \cdot r)}}^{\text{let } \mathcal{S}_c(s, r)} \cdot r, \hspace{5mm} \frac{\partial c}{\partial r} = \mathcal{S}_c(s, r) \cdot s
\end{equation}

\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{\plotpath/cost.pdf}
    \caption{Softplus cost function for different $r$ and fixed $s = 4$ and $c_0 = 2$}
    \label{fig:costs}
\end{figure}
