...
Using this property we can find an approximation of the policy function (see \ref{a:limiting}). Assuming positive unit profits, $p_t > k$,

\begin{equation} \label{rpolicy}
    r(s_t, p_t) = \begin{cases}
        \frac{1 - \beta}{\beta} \cdot s_t - \frac{1}{2} \sqrt{ \left(\frac{1 - \beta}{\beta} \cdot 2 s_t \right)^2 - 4 \cdot \left(p_t - k\right) } & \text{ if } s_t > \frac{\beta}{1-\beta} \sqrt{p_t - k} \\
        \frac{\beta}{1-\beta} \sqrt{p_t - k} - s_t                                                                                                  & \text{ otherwise }
    \end{cases}
\end{equation}

On the other hand, if $p_t < k$, then,

\begin{equation}
    r(s_t, p_t) = -\gamma \cdot s_t.
\end{equation}

This function is plotted in Figure (\ref{fig:r}).
...
Given the ramp-up policy function $r(s, p)$, it is possible to find the \textit{excess} demand of electricity in the local market, that is, the electricity demanded in the local market by local prosumers that it is not satisfied by local producers.
...
\subsubsection{Interpretation of the policy function}

To get a better understanding of the policy rule (Equation \ref{local_p}), assume that $p_t$ is at a level that keeps (in the eyes of the provider) the electricity production constant, namely,

\begin{equation}
    \tilde{R}(p_t) = 0 \iff p_t = -\frac{a_t}{b_t}
\end{equation}

Then $p_{t+1} = \left( \frac{1 - \beta}{\beta} \right) \cdot \frac{X_t}{b_t} + 2P^m(\matr{Y}_{t+1})$.

First, a clear relationship is that between $X_t$ and $p_{t+1}$. If there is excess demand in the local market, $X_t > 0$, then an increase in its magnitude yields an increase of the price in order to increase production. Likewise, if $X_t < 0$, an increase in the excess supply leads to a decrease in the price to lower electricity production. It is important to notice that the provider sets the price $p_{t+1}$, based on data at time $t$, which affects the electricity production at time $t+1$. Second, assume without loss of generality $X_t > 0$. If the cost of acquiring electricity from abroad is expected to increase ($P^m(\matr{Y}_{t+1}) \uparrow$), the producer will increase the price in the local market, $p_{t+1}$, in order to increase local production.
...
Assume the demand is locally satisfied,

\begin{equation}
    M \cdot e_t = S_t: X_t = 0
\end{equation}

Furthermore, assume we are in a constant price equilibrium, $p_t = p_{t+1} = k$. The local price policy function implies that stability is sustained only if

\begin{equation}
    \frac{a_t}{b_t} = 2P^m(\Y_{t+1}),
\end{equation}