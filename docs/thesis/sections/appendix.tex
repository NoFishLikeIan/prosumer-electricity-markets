...
\begin{equation*}
    \begin{split}
        \mathcal{L}(p, \Y, X) &= p \  X - \sum_{j \in N_{\mathcal{A}}(i)} Y^j_t \  P^j(\Y) \\
        &+ \lambda_t \  \left( X -  \sum_{j} Y_t^j \right) \\
        &+ \beta \   V\left(X - \tilde{R} (p) \right)
    \end{split}
\end{equation*}
...
\begin{equation} \label{foc_p}
    \begin{split}
        \frac{\partial}{\partial p} \mathcal{L} &= X - \beta \  V^\prime \left(X - \tilde{R}(p) \right) \  \tilde{R}^\prime(p) = 0 \\
        X &= \beta \  V^\prime \left(X - \tilde{R}(p) \right) \  \frac{\partial \tilde{R}}{\partial p}(p)
    \end{split}
\end{equation}

This first order condition implies that the benefit of charging a higher price today, which is today's demand $X$, should be equal to the cost of reducing the demand tomorrow, $\beta \  V^\prime \left(X - \tilde{R}(p) \right) \  \frac{\partial \tilde{R}}{\partial p}(p)$. The second first order condition requires that, for each neighbor $m$, 
...
\begin{equation} \label{env}
    V^\prime(X_t) = p_t + \lambda_t + \beta \  V^\prime(X_{t+1})
\end{equation}
...
\begin{equation}
    \frac{X_t}{\beta \  \frac{\partial \tilde{R}}{\partial p}(p_t)} = p_{t+1} + \lambda_{t+1} + \frac{X_{t} - \tilde{R}(p_t)}{\frac{\partial \tilde{R}}{\partial p}(p_{t+1})}
\end{equation}
...
\begin{equation}
    \begin{split}
        \frac{X_t}{\beta \  b} &=  \lambda_{t+1} + p_{t+1} + \frac{X_t}{b} - \frac{a +  b \  p_{t}}{b} \\
        p_{t+1} &= \left( p_t + \frac{a}{b} \right) +  \left( \frac{1 - \beta}{\beta} \right) \  \frac{X_t}{b} - \lambda_{t+1}
    \end{split}
\end{equation} 
