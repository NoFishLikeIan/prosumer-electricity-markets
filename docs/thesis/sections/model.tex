...
\begin{equation} \label{raw_policy_producer}
    \begin{split}
        ...
        \\
        mc(r; s_t, p_t) &\coloneqq \frac{\partial c}{\partial r}(s_t, r) \cdot r + c(s_t, r) \\
        \hfill &= \hfill \\
        \beta \cdot mb(r; s_t, p_t)  &\coloneqq \beta \cdot \left\{p_t - k + \left[\frac{\partial c}{\partial r}(s_t + r, r) - \frac{\partial c}{\partial s_t}(s_t + r, r)\right] \cdot r + c(s_t + r, r) \right\}
    \end{split}
\end{equation}
...
Notice that $c(s, r; c_1) \xrightarrow{} \max \{ 0, s \cdot r \}$ as $c_1 \xrightarrow{}\infty$. Hence, at the limit $c_1 \xrightarrow{}\infty$, we can define the policy function  $r(s_t, p_t)$ analytically (see \ref{a:limiting}). This ``asymptotic'' policy function is continuous everywhere but differentiable only within three intervals. The first trivial interval to consider is the space where $p_t < k$. Here producers make a loss at each unit sold, or in terms of the first order condition $mc(r) > \beta \cdot mb(r)$. They will then try and reduce production as much as possible, namely \begin{equation*} r(s_t, p_t) = -\gamma \cdot s_t. \end{equation*} The second interval is that were current electricity production is too low, namely $s_t < \frac{\beta}{1- \beta} \sqrt{p_t - k}$, which implies $mc(r) < \beta \cdot mb(r)$) in the first order condition. In this case, ramp-up will be as high as possible $r(s_t, p_t) = \overline{r}$. Finally, the interesting case is that of the remaining interval where $mc(r) = \beta \cdot mb(r)$. As shown in Appendix \ref{a:limiting},
...
Note that if $S_t > M \cdot e_t$, there is an excess supply ($X_t < 0$) of electricity in the local market that the local provider can use to sell abroad. Given equation (\ref{x_inst}), we obtain the evolution of the process $X_t$ (see \ref{a:ev_demand}),
...
The condition $X_{i, t}(p_{i, t-1}) =  \sum_{j \in N_{\mathcal{A}}(i)} Y_t^{(i, j)}$ requires the provider to always match (sell) the quantity demanded (supplied) in the local market by trading with its neighbors. Note that this implies that electricity across the network flows always towards places of scarcity but in doing so it disrupts the pricing across the network. In this way, electricity prices signal both scarcity and bargaining power within the network.

The provider's optimization is then an intertemporal problem that depends on the state $X_t$ which follows the evolution layed down in equation (\ref{x_true}). The provider is assumed to make two simplifying assumptions on this evolution. First, at time $t$, they assumes $\E_t\left[M \cdot (\E[e_{t+1}] - e_{t}) \right] = 0$. This assumption is a good approximation in the case of a very persistent process $e_t$. Second, I assume the provider does not know the ramp-up function of producers in the local market, $r(s_t, p_t)$, hence they approximate the aggregate ramp-up function by a linear adaptive rule, proportional to the price, namely $\E_t \left[R_t(p) \right] = a_t + b_t \cdot p$ with $a < 0$ and $b > 0$.
...
This first order condition requires that the marginal benefit of exploiting current excess demand $X$ should equal the perceived marginal benefit loss, $\beta \cdot V^\prime \left(X - \left( a_t + b_t \cdot p_t \right) \right)$, of reducing excess demand next period, $b_t$. On the other hand, the first order condition with respect to an arbitrary traded quantity $Y^j_t$ is,
...
\begin{equation} \label{local_p}
    p_{t+1} = p_t + \frac{a_t}{b_t} +  \left( \frac{1 - \beta}{\beta} \right) \cdot \frac{X_t}{b_t} - \lambda_t
\end{equation}
...
\begin{figure}[!ht]
    ...
    \input{sections/diagrams/linemodel.tikz}
\end{figure}
The sequential graph example is illustrative but in general there is no analytical way to go from $\matr{A}$ and $\matr{G}$ and one has to compute it algorithmically on a case by case basis.

Now, let $\Delta_t$ in $\R^{\abs{E}}$ be the vector of revenue difference in the respective local markets with entries,

\begin{equation}
    \Delta^{(i, j)}_t \coloneqq X_{i, t} \cdot p_{i, t} - X_{j, t} \cdot p_{j, t}
\end{equation}

Given $\matr{G}$ and $\Delta^{(i, j)}_t$, at each step $t$, equation (\ref{bargaining_solution}) can be written as,
...
\begin{equation} \label{matrix_bargaining_solution}
    \begin{split}
        ...
        P &= \left((2\matr{I} + \matr{G})^{-1} \Delta \right) \oslash \Y
    \end{split}
\end{equation}
...
\begin{equation} \label{Pmatrix}
    P(\Y_t) = \left((2\matr{I} + \matr{G})^{-1} \Delta_t \right) \oslash \Y_t
\end{equation}
...
This can be also seen by looking at the two $\det(2 \matr{I} + \matr{G})$,
...
Because of this limitation, here I will limit the analysis to acyclic (or complete) graphs. Extending this model to cyclical graphs simply requires defining a more general once differentiable function $P: \R^{\abs{E}} \mapsto \R^{\abs{E}}$ that models the bargaining procedure.
...
This equation determines the ratio of $Y^{(i, m)}_t / Y^{(i, n)}_t$ for each element of the vector of $\matr{Y}_t$. Furthermore, to satisfy the resource constraint, it needs to be true that, $\sum_{j} Y^{(i, j)}_t = X_{i, t}$. These two equations allow to fully determine the vector $\matr{Y}_t$.
...
\begin{equation}
    \E_t \left[R_{t+1}(p_t) \right] = a_t + b_t \cdot p_t
\end{equation}
