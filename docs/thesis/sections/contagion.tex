...
\begin{equation*}
    P^{(i, j)}_t = \frac{\sum_{(l, m) \in E} \Delta^{(l, m)}_t \cdot (2\I + \G)^{-1}_{(i, j), (l, m)}}{Y^{(i, j)}_t},
\end{equation*}
...
\begin{equation}
    \begin{split}
        \frac{P^{(i, j)}_{t}}{\partial X_{k, t}} &= \frac{1}{Y^{(i, j)}_t} \left( p_{k, t} \cdot \sum_{(k, m) \in E} (2\I + \G)^{-1}_{(i, j), (k, m)} - p_{k, t} \cdot \sum_{(l, k) \in E} (2\I + \G)^{-1}_{(i, j), (l, k)} \right) \\
...
\begin{equation*}
    \rho = \sqrt{\frac{\left(1 - \frac{2}{3}\right)^2 + 2 \cdot \left(\frac{1}{2} - \frac{2}{3}\right)^2}{3 - 1}} \approx 2.89.
\end{equation*}
...
Then we can simulate the average blackout, $ \frac{\sum_{t \geq \tau, i} X_{i, t}}{(T - \tau) \cdot n}$, after a given shock for different network structures. Figure (\ref{fig:blackout}) shows how the size of the blackout is decreasing in graph incoherence.
