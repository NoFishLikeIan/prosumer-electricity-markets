...
The equation has a few intuitive properties. First of all, there is no ramp-up if there is no unit profit, that is $r(s_t, p_t) = 0$ if $p_t - k = 0$. Second, ramp-up increases in $s_t$ and decreases in $p_t$. The function is plotted in Figure $\ref{fig:r}$, where I choose $\underline{r}(s) = - \delta s$ with $\delta \in (0, 1)$ and $\overline{r}(s) = \sqrt{p-k}$.
...
\begin{equation} \label{local_p_comp}
  p_{t+1} = p_t + \frac{1-\beta}{\beta \ \gamma} X_t + \frac{\alpha + \eta S_t}{\gamma} + 2P^m(\matr{Y}_{t})
\end{equation}
