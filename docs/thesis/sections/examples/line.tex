...
Using the definition of Penn number

\begin{equation}
    P_m = \frac{(1 + \sqrt{2})^m - (1 - \sqrt{2})^m}{2\sqrt{2}}
\end{equation}
...
Using some manipulation $P_{-a} = (-1)^{a+1} P_a$ and $P_{a+b} = P_{a} P_{b+1} + P_{a-1} P_b$,

\begin{equation}
    \lim_{n \xrightarrow{} \infty} H(i, j, n) = (-1)^{j + 1}  \left(P_j \cdot \gamma - P_{j+1} \right) P_i
\end{equation}
