...
Clearly, the ramp-up policy function will depend entirely on the choice of the marginal cost function, $c(s, r)$. I assume that ramp-down of electricity production has no marginal costs, namely $c(s, r) = 0$ if $r < 0$, and should have increasing marginal costs both in scale of supply, $\partial c / \partial s > 0$, and in ramp-up level, $\partial c / \partial r > 0$. A differentiable function with (asymptotically) this feature is
...
\begin{figure}[H]
  ...
  \caption{The ramp-up function $r$ of a producer given its current electricity production level $s_t$ and the electricity price $p_t$}
  \label{fig:r}
\end{figure}
...
Note that $\lambda_t$ needs to be the same for every neighbor, such that, for two neighbors of $i$, $m$ and $n$
...
Having derived the budget constraint on the bargained quantity, we can break up the policy function (\ref{local_p}) into a local effect $L_t(X_t, S_t)$ and a global effect $2P^m(\matr{Y}_{t})$, namely
...
\begin{equation}
  \B_{t} \left[R(p_t, S_t) \right] = \alpha_{t} + \gamma_{t} \  p_t + \eta_{t} \ S_t.
\end{equation}
...
Table \ref{table:perception} recaps the belief structure of the model.
