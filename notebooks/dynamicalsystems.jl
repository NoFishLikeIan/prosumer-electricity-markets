...
# ╔═╡ eea2c833-cda1-40dd-98c1-4a8f0bc49f36
begin
	p₀ = k .+ [0.5, 0.5]
	S₀ = copy(e)
...
# ╔═╡ f9047aea-0d88-4dc8-839f-a39f1416af16
begin
	t, ps, Ss = maketrajectory(ds; t = 100)
	
	trajfig = plot(xaxis = plabel, yaxis = Slabel, zaxis = tlabel)

	for i in [1, 2]
		plot!(trajfig, ps[:, i], Ss[:, i], t, label = i)
	end
...
begin	
	...
	for i in [1, 2]
		plot!(supplyplot, t, Ss[:, i], label = i)
	end
	
		
	for i in [1, 2]
		plot!(priceplot, t, ps[:, i], label = i)
	end
...
# ╔═╡ 3b2bde1d-d0b6-4886-9431-9ca0c23941d9
if false
	
	pg, Xg, basins, attractors = makebasins(ds)
	
	figure = heatmap(pg, Xg, basins, xaxis = plabel, yaxis = Xlabel)
