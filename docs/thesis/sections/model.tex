...
The condition $X_{i, t}(p_{i, t-1}) =  \sum_{j \in N_{\mathcal{A}}(i)} Y_t^{(i, j)}$ requires the provider to always match (sell) the quantity demanded (supplied) in the local market by trading with its neighbors. For no, assume every pairwise price on the network, $P^{(i, j)}$, is determined only by the vector of traded quantities, $\Y$.
...
\begin{equation} \label{matrix_bargaining_solution}
    \begin{split}
        ...
        P &= (2\matr{I} + \matr{G})^{-1} (\Delta  \oslash Y)
    \end{split}
\end{equation}

where $\circ$ and $\oslash$ denote the element-wise (Hadamard) product and division respectively. To get a better intuition for this result, we can use the Neumann expansion of $(2\matr{I} + \matr{G})^{-1}$,

\begin{equation}
    P = \sum^{\infty}_{k=0}\frac{(-1)^k}{2^{k+1}} \matr{G}^k (\Delta  \oslash Y)
\end{equation}

Hence, the bargaining solution gives the price following price formation,

\begin{equation}
    P(\Y_t) = (2\matr{I} + \matr{G})^{-1} (\Delta _t \oslash \Y_t)
\end{equation}

with associated Jacobian (see \ref{a:jacobian_p}),

\begin{equation}
    \mathbb{J}\{P\}(\Y_t) = - (2\matr{I} + \matr{G})^{-1} \diag (\Delta _t \oslash \Y^2_t)
\end{equation}
...
The write hand side can be written as a vector over the edges by (see appendix \ref{a:shadow_vector})

\begin{equation}
    \begin{split}
        &P + \mathbb{J}\{P\} ( \matr{I} + \matr{G} ) \Y \\
        &(2\matr{I} + \matr{G})^{-1} \diag(\Delta  \oslash \Y) \iota - (2\matr{I} + \matr{G})^{-1} \diag (\Delta  \oslash \Y^2) ( \matr{I} + \matr{G} ) \Y \\
        &(2\matr{I} + \matr{G})^{-1} \diag(\Delta  \oslash \Y) \left(\iota - \diag(\Y)^{-1} ( \matr{I} + \matr{G} ) \Y \right) \\
        &(2\matr{I} + \matr{G})^{-1} \diag(\Delta  \oslash \Y) \left( \iota - (\matr{I} + \matr{G}) \iota \right) \\
        &(2\matr{I} + \matr{G})^{-1} \diag(\Delta  \oslash \Y) \matr{G} \iota
    \end{split}
\end{equation}