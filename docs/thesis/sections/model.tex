...
\begin{figure}[!ht]
    ...
    \input{sections/diagrams/linemodel.tikz}
\end{figure}
The sequential graph example is illustrative but in general there is no analytical way to go from $\matr{A}$ and $\G$ and one has to compute it algorithmically on a case by case basis.
...
Given $\G$ and $\Delta^{(i, j)}_t$, at each step $t$, equation (\ref{bargaining_solution}) can be written as,
